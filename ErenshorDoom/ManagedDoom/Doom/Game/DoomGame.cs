...
namespace ManagedDoom
{
	public sealed class DoomGame
	{
		...
		{
			options.Skill = (GameSkill)MathCompat.Clamp((int)skill, (int)GameSkill.Baby, (int)GameSkill.Nightmare);

			if (options.GameMode == GameMode.Retail)
			{
				options.Episode = MathCompat.Clamp(episode, 1, 4);
			}
			...
			if (options.GameMode == GameMode.Commercial)
			{
				options.Map = MathCompat.Clamp(map, 1, 32);
			}
			else
			{
				options.Map = MathCompat.Clamp(map, 1, 9);
			}
